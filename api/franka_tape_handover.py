...
arm1_handover_pos = handover_pos.copy()
# Need a way to get the width of the yellow tape that isnt privileged
# this is half the width of the franka gripper:
arm1_handover_pos[1] -= 0.1025 # shift handover position right by half the width of the franka gripper
arm1_handover_pos[0] += 0.02 # shift handover position forward by 2cm
...
# Arm0: pick up yellow tape
open_gripper_arm0()
# shift the pick -y by 2cm to grab the tape on one end of the radius
goto_pose_arm0((yellow_tape_pos+np.array([-0.01, 0.05, -0.01])), gripper_down_quat, z_approach=0.15)
close_gripper_arm0()
...
goto_pose_arm0(handover_pos, gripper_rotated_side_quat)

# Arm1 approach
# arm1_quat = np.array([0.707, 0.707, 0, 0])
arm1_quat = gripper_side_quat
open_gripper_arm1()
goto_pose_arm1(arm1_handover_pos, arm1_quat, z_approach=0.10)
close_gripper_arm1()
...
shifted_arm1_pos = arm1_handover_pos + vtf.SO3(wxyz=arm1_quat).as_matrix() @ np.array([0, 0, -0.1])
goto_pose_arm1(shifted_arm1_pos, arm1_quat)
goto_pose_arm0(shifted_handover_pos, gripper_rotated_side_quat)
goto_home_joint_position_arm0()
goto_home_joint_position_arm1()

# Arm1: drop cube in bowl, shifted to the left because the tape is slightly off-center in the robot's grasp
goto_pose_arm1((duct_tape_pos+np.array([0, -0.05, 0.05])), gripper_down_quat, z_approach=0.15)
open_gripper_arm1()
goto_pose_arm1((duct_tape_pos+np.array([0, -0.05, 0.2])), gripper_down_quat)
goto_home_joint_position_arm1()
"""
...
# Need a way to get the width of the yellow tape that isnt privileged
# this is half the width of the franka gripper:
arm1_handover_pos[2] += 0.1025
arm1_handover_pos[1] += 0.01
...
class FrankaTapeHandoverCodeEnv(CodeExecutionEnvBase):
    ...
    oracle_code = ORACLE_CODE_SWAPPED
